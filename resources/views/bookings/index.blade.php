        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-100 pb-8">
            ...
            <div class="flex-shrink-0">
                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-50 text-[#4F46E5]">
                    {{ $bookings->count() }} Total {{ Str::plural('Reservation', $bookings->count()) }}
                </span>
            </div>
        </div>

        @if($bookings->isEmpty())
            <div class="flex flex-col items-center justify-center py-24 text-center" id="empty-bookings-state">
        ...
        @else
            <div class="space-y-6" id="bookings-list">
                @foreach($bookings as $booking)
                    ...
                    @if($event)
                        <div class="premium-card rounded-2xl p-6 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 hover:shadow-md transition-shadow {{ $booking->status === 'cancelled' ? 'opacity-60' : '' }}">
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5 w-full lg:w-auto">
                                <div class="w-full sm:w-28 h-20 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0">
                                    <img src="{{ $event->image ?: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&w=300&q=80' }}" alt="{{ $event->title }}" class="w-full h-full object-cover">
                                </div>
                                <div class="space-y-1.5">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @endif
                                        <span class="text-xs text-slate-400 font-medium">Booking ID: #EVB-{{ $booking->id }}</span>
                                    </div>
                                    <h3 class="text-base font-bold text-slate-900 hover:text-[#4F46E5] transition">
                            ...
                            <div class="flex flex-row lg:flex-col items-center lg:items-end justify-between w-full lg:w-auto border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-100 gap-4">
                                <div class="text-left lg:text-right">
                                    ...
                                    <span class="text-base font-extrabold text-[#4F46E5]">{{ $event->price == 0 ? 'Free' : '$' . number_format($event->price, 2) }}</span>
                                </div>
                                <div class="flex items-center space-x-2">
                                    @if($booking->status === 'confirmed')
                                        <form method="POST" action="/bookings/{{ $booking->id }}/cancel" class="inline-block">
                                            @csrf
                                            <button type="submit" class="px-3.5 py-2 rounded-xl text-xs font-semibold text-rose-500 bg-rose-50 hover:bg-rose-100/60 transition">
                                                Cancel Booking
                                            </button>
                                        </form>
                                    @else
